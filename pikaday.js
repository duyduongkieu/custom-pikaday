...
(function(root, factory) {
  ...
  var hasMoment = typeof moment === 'function',
    ...
    renderHead = function(opts) {
      ...
      return (
        '<thead>' + (opts.isRTL ? arr.reverse() : arr).join('') + '</thead>'
      );
    },
      ...
      if (opts.splitTimeView) {
        ...
        for (var h = 0; h < 24; h++) {
          for (var m = 0; m < 60; m += minutesStep) {
            ...
            disabled =
              (isMinDate && date <= self._minTime) ||
              (isMaxDate && date >= self._maxTime);
        ...
        results =
          ...
          renderTimePicker(
            60,
            minutesStep,
            round(self._minutes, minutesStep),
            'pika-select-minutes'
          );
        ...
        if (opts.showSeconds) {
          results +=
            ...
            renderTimePicker(
              60,
              secondsStep,
              round(self._seconds, secondsStep),
              'pika-select-seconds'
            );
        }
        ...
        if (!h24) {
          results +=
            ' <select class="pika-select picka-select-ampm"><option value="AM" ' +
            (self._hours < 12 ? 'selected' : '') +
    ...
    Pikaday = function(options) {
      ...
      self._onMouseDown = function(e) {
        ...
        if (
          !(
            hasClass(target, 'is-disabled') ||
            hasClass(target.parentElement, 'is-disabled')
          )
        ) {
          if (
      ...
      self._onChange = function(e) {
        ...
        if (hasClass(target, 'pika-select-month')) {
          ...
          if (!self._d) {
            var date = isDate(self._o.defaultDate)
              ? self._o.defaultDate
              : new Date();
  ...
  Pikaday.prototype = {
    ...
    config: function(options) {
      ...
      self._o.field =
        self._o.field && self._o.field.nodeName ? self._o.field : null;

      self._o.bound = !!(self._o.bound !== undefined
        ? self._o.field && self._o.bound
        : self._o.field);

      self._o.trigger =
        self._o.trigger && self._o.trigger.nodeName
          ? self._o.trigger
          : self._o.field;

      self._o.disableWeekends = !!self._o.disableWeekends;

      self._o.disableDayFn =
        typeof self._o.disableDayFn == 'function' ? self._o.disableDayFn : null;
      ...
      if (
        self._o.minDate &&
        self._o.maxDate &&
        self._o.maxDate < self._o.minDate
      ) {
    ...
    toString: function(format) {
      ...
      if (this._o.showTime) {
        ...
        timeString =
          ...
          h +
          ':' +
          zeroFill(this._minutes) +
          (opts.showSeconds ? ':' + zeroFill(this._seconds) : '') +
          (h24 ? '' : ' ' + this._amPm);
      }
    ...
    _onDateTimeDidChange: function() {
      if (this._o.field) {
        this._o.field.value = isDate(this._d)
          ? this.toString()
          : this._o.defaultText;
        fireEvent(this._o.field, 'change', { firedBy: this });
      }
